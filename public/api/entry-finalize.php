...
try {
    ...
    $stage = strtoupper((string) ($entry['workflow_stage'] ?? ''));
    if ($stage !== 'COMPLETE') {
        http_response_code(409);
        echo json_encode(['ok' => false, 'error' => 'Entry must be COMPLETE before finalizing']);
        exit;
    }
    if (!$repo->hasRequiredMetadata($entryUid)) {
        http_response_code(409);
        echo json_encode(['ok' => false, 'error' => 'Metadata is still processing; finalize is not available yet']);
        exit;
    }

    $repo->setStageForUser($entryUid, $userId, 'FINAL');

    echo json_encode(['ok' => true, 'entry_uid' => $entryUid, 'stage' => 'FINAL']);
} catch (Throwable $throwable) {
    http_response_code(500);
    echo json_encode(['ok' => false, 'error' => 'Unable to finalize entry']);
}
